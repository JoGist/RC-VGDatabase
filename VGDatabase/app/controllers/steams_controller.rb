    ...
    def auth_callback
      ...
      if session[:user_id]==nil
        if !User.exists?(:steam_token => auth.uid)
          User.create(:username => auth.info['nickname'],:steam_username => auth.info.values[4].values[0].split('id/')[1].split('/')[0],:avatar => auth.extra.raw_info['avatarfull'],:steam_token => auth.uid,:background => 'deafult.png')
